...
def SIFT(src):
    ...
    for i in range(len(keypoints)):
        ...
        ## keypoints[i].angle = ???
        ###################################################################
        # histogram에서 최대 값에 해당하는 angle 탐색
        max_value_index = np.argmax(orient_hist).astype(np.float32)
        keypoints[i].angle = max_value_index * 10

        # histogram에서 최대 값 * 0.8 이상에 해당하는 angle도 탐색
        for j in range(36):
            if (j != max_value_index) and (orient_hist[j] > orient_hist[int(max_value_index)] * 0.8):
    ...
    for i in range(len(keypoints)):
        ...
        for row in range(-8, 8): # 16
            for col in range(-8, 8): # 16
                # 회전을 고려한 point값을 얻어냄
                ...
                patch_row_index = int((row + 8) // 4)
                patch_col_index = int((col + 8) // 4)
                hist_index = patch_row_index * 4 + patch_col_index

                if int(descriptors_angle[p_y, p_x] // 45) == 8 : # index error 방지를 위한 코드
                    continue
                else :
                    other_orient_hist[hist_index * 8 + int(descriptors_angle[p_y, p_x] // 45)] += magnitude[p_y, p_x] * gaussian_weight

        # 마지막에 descriptor[i] = histogram을 넣어준다.
...
def main():
    ...
    # 결과의 학번 작성하기!
    cv2.imshow('201701971_my_sift', result)
    cv2.waitKey()
